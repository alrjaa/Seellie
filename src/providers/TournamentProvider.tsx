...
export interface TournamentContextType {
  ...
  /** تفعيل مسار ثانٍ واحد: منظم أو مواهب (مع المتابع) */
  enableSecondaryRole: (
    role: SecondaryRole,
  ...
  addPlayerToTeam: (
    ...
    teamId: string,
    playerData: {
      /** إيميل أو @معرف أو كود تسجيل أو جوال لحساب متابع موجود */
      identifier: string;
      jerseyNumber: number;
...
export function TournamentProvider({ children }: { children: ReactNode }) {
  ...
  const profilesSyncLock = useRef(createInFlightLock<number>());
  ...
  const resolveRosterFollowerAccount = useCallback(
    (identifier: string): User | null => {
      const account = resolveFollowerByIdentifier(
        usersRef.current,
        identifier
      );
      if (!account) {
        toast({
          variant: 'destructive',
          title: t('organizer.competitionManage.rosterAccountRequiredTitle'),
          description: t('organizer.competitionManage.rosterAccountNotFound'),
        });
        return null;
      }
      return account;
  ...
  /** ترحيل البيانات القديمة فقط — ليس لمسار التسجيل الجديد */
  ...
  useEffect(() => {
    ...
    const applyLocalStoredUser = (stored: User) => {
      if (isLegacyLocalDemoAdmin(stored)) {
        void removeJson(USER_STORAGE_KEY);
        setCurrentUser(null);
        return;
      }
      if (!stored.email || !stored.role) return;
      try {
        const match =
          initialUsers.find(
        ...
        const base = match
          ? {
              ...
              activeRole: stored.activeRole ?? match.activeRole ?? match.role,
    ...
    (async () => {
      // ── Phase A: auth-only path (must unlock Login UI quickly) ──────────
      try {
        await migrateLocalSessionCredentials();
        const stored = await getJson<User>(USER_STORAGE_KEY);
        if (!active) return;
        if (stored) applyLocalStoredUser(stored);
        ...
        // Unlock Login / Index regardless of catalog hydrate
        if (active) setLoading(false);
      }

      // ── Phase B: non-auth local caches (must NOT block Login) ───────────
      try {
        const [
          storedLogo,
          storedName,
        ...
        console.warn('catalog bootstrap failed', error);
      }
    })();
    return () => {
      active = false;
  ...
  const registerRefereeForCompetition = useCallback(
    (
      ...
      const account = resolveRosterFollowerAccount(data.identifier);
      if (!account) return false;

      const existing = referees.find((r) => r.id === account.id);
      if (existing) {
        ...
        setCompetitions((prev) => {
          const next = prev.map((c) => {
            ...
            return {
              ...c,
              refereeIds: [...new Set([...c.refereeIds, existing.id])],
            };
          });
      ...
      setReferees((prev) => {
        if (prev.some((r) => r.id === account.id)) return prev;
        const next = [...prev, referee];
        void (async () => {
  ...
  const refreshCloudMessages = useCallback(async () => {
    ...
    await messagesSyncLock.current.run(async () => {
      ...
      if (arrived.length && sessionUserIdRef.current === uid) {
  ...
  useEffect(() => {
    ...
    const stop = subscribeShareCardsForUser(uid, (card, event) => {
      ...
      setShareCards((prev) => {
        if (sessionUserIdRef.current !== uid) return prev;
        return mergeShareCardsById([card], prev);
      });
    });
  ...
  useEffect(() => {
    ...
    void syncCloudUsers();
    const stopPoll = startForegroundInterval(SYNC_FALLBACK_MS.profiles, () => {
      void syncCloudUsers();
    });
    return () => {
      stop?.();
  ...
  const addPlayerToTeam = useCallback(
    (
      ...
      const account = resolveRosterFollowerAccount(playerData.identifier);
      if (!account) return false;
        ...
        const next = prev.map((c) => {
          ...
          return {
            ...
            teams: c.teams.map((team) => {
              ...
              if (team.players.some((p) => p.id === account.id)) {
                alreadyOnTeam = true;
                return team;
              }
              added = true;
      ...
      if (alreadyOnTeam) {
        toast({
          variant: 'destructive',
          title: t('toasts.t045_e1da8e'),
          description: t('organizer.competitionManage.rosterAlreadyOnTeam', {
            name: account.name,
          }),
        });
        return false;
    ...
    [toast, t, resolveRosterFollowerAccount, syncCompetitions]
  );
  ...
  const addStaffToCompetition = useCallback(
    (
      ...
      const account = resolveRosterFollowerAccount(staffData.identifier);
      if (!account) return false;
      ...
      setCompetitions((prev) => {
        const next = prev.map((c) => {
          ...
          return {
            ...
            teams,
            staff: [
              ...(c.staff || []).filter((s) => s.id !== account.id),
              staffEntry,
            ],
          };
        });
